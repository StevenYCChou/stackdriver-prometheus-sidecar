...
var (
	sortedAutoGeneratedMetricNames = []string{
...
func (s *shardCollection) enqueue(hash uint64, sample *monitoring_pb.TimeSeries) {
	...
	queueLength.WithLabelValues(strconv.FormatUint(shardIndex, 10)).Inc()
	
	metricType := sample.GetMetric().GetType()

	if shardIndex == 0 {
		level.Debug(s.qm.logger).Log("msg", "enqueue with shard Index 0",
									 "hash", hash,
									 "metric_type", metricType)
	}

	promMetricName := strings.TrimPrefix(sample.GetMetric().GetType(), "kubernetes.io/anthos/")
	if idx := sort.SearchStrings(sortedAutoGeneratedMetricNames, promMetricName); idx < len(sortedAutoGeneratedMetricNames) && sortedAutoGeneratedMetricNames[idx] == promMetricName {
		return
	}
...
func (s *shardCollection) runShard(i int) {
	...
	for {
		...
		case entry, ok := <-shard.queue:
			...
			queueLength.WithLabelValues(strconv.FormatInt(int64(i), 10)).Dec()
